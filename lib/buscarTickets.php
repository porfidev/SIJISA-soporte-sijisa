...
if (isset($_POST["inIdTicket"])) {
  if ($_POST["inIdTicket"] != "") {
    ...
    $oTicket->setValores(["id_unico" => $idticket]);
    $tickets = $oTicket->consultaTicket();

    if (!empty($tickets)) {
      $oEmpresa->isQuery("id");
      $oEmpresa->setValores(["id_empresa" => $tickets[0]["intIdEmpresa"]]);
...
if (isset($_POST["inEmpresaTicket"]) and $_POST["inEmpresaTicket"] != "") {
  $idempresa = filter_var(
    $_POST["inEmpresaTicket"],
    FILTER_SANITIZE_NUMBER_INT
  );

  $oTicket->getByCompany($idempresa);
  $tickets = $oTicket->consultaTicket();

  echo json_encode(["success" => true, "tickets" => $tickets]);
  die();

  $_POST["inPrioridad"] = empty($_POST["inPrioridad"])
    ? []
    : $_POST["inPrioridad"];

  settype($idempresa, "integer");

  $tickets = $oTicket->isSearch(
    $idempresa,
    $_POST["inPrioridad"],
    $_POST["inEstado"],
    $_POST["inFechaInicio"],
...
if (!empty($tickets)) {
  ...
  foreach ($tickets as $campo) {
    ...
    $valores[$i][] = $empresa;
    $valores[$i][] =
      "<a href='#myModal' role='button' class='detalleTicket' data-toggle='modal' data-backdrop='static' data-id='" .
      $campo["intIdTicket"] .
...
if (!empty($tickets)) {
  $respuesta = ["tickets" => true, "datos" => $valores];
} else {
  $respuesta = ["tickets" => false, "status" => $status, "mensaje" => $mensaje];
}
