...
class UsuarioDEPRECATED
{
  ...
  public function hacerLogin($parametros = [])
  {
    if (isset($parametros["usuario"]) and isset($parametros["contrasena"])) {
      $consulta =
        "SELECT * FROM usuarios WHERE username = :usuario AND password = :contrasena";
    }
   ...
   * @return bool
   */
  public function obtenerUsuario($parametros = [])
  {
    $valores = null;
    ...
    if (isset($parametros["empresa"])) {
      ...
      $valores = ["empresa" => $parametros["empresa"]];
    }
...
class EmpresaDEPRECATED
{
  ...
  public function obtenerEmpresa($parametros = [])
  {
    $valores = null;
  ...
  public function registrarEmpresa($parametros = [])
  {
    $valores = [];
    $consulta = "INSERT INTO catempresas VALUES (null,:nombre,:siglas,:email)";

    $valores["nombre"] = $parametros["nombre"];
    $valores["siglas"] = isset($parametros["siglas"])
      ? $parametros["siglas"]
      : null;
    $valores["email"] = isset($parametros["email"])
      ? $parametros["email"]
      : null;
...
class TicketDEPRECATED
{
  ...
  public function registrarTicket($parametros = [])
  {
    //recibe ['datos_formulario']=>$_POST y ['archivo_adjunto']=>$_POST
    ...
    $empresa = $oDatosEmpresa->obtenerEmpresa([
    ...
    foreach ($empresa as $indice => $campo) {
      $siglas =
        $campo["siglasEmpresa"] != ""
          ? $campo["siglasEmpresa"]
          : strtoupper(substr($campo["Descripción"], 0, 3));
      $id_unico =
        substr($parametros["datos_formulario"]["tipo_solicitud"], 0, 1) .
        "-" .
        $siglas .
        "-" .
        date("Ymd") .
    ...
    if (
      isset($parametros["archivo_adjunto"]) and
      !empty($parametros["archivo_adjunto"])
    ) {
  ...
  public function actualizarTicket($param = [])
  {
    if (
      isset($param["archivo_adjunto"]) and !empty($param["archivo_adjunto"])
    ) {
    ...
    $consulta =
      "UPDATE tickets SET prioridad = :prioridad, intIdEstatus = :estatus WHERE intIdTicket = :idticket";
    $valores = [
      "prioridad" => $param[0]["prioridad"],
      "estatus" => $param[0]["estado"],
      "idticket" => $param[0]["id_ticket"],
    ...
    if ($param[0]["estado"] == 2) {
      ...
      $valores = [
        "fecha_asignacion" => self::timeZone(),
        "idticket" => $param[0]["id_ticket"],
      ];
  ...
  public function obtenerTickets($parametros = [])
  {
    $valores = null;
    ...
    if (isset($parametros["id_ticket"])) {
      ...
      $valores = ["ticket" => $parametros["id_ticket"]];
    }
  ...
  public function obtenerSeguimientoTicket($param = [])
  {
    $consulta = "SELECT transiciones.*, catusuarios.nombre
					...
					ORDER BY fecha DESC";
    $valores = ["ticket" => $param["id_ticket"]];
  ...
  public function moverArchivos($archivos)
  {
    //Optimizada para multiples archivos
      ...
      foreach ($archivos["archivo_adjunto"]["error"] as $key => $error) {
        if ($error == 0) {
          ...
          if ($error != 0 and $error != 4) {
            //Si no subieron archivos No enviar ninguna advertencia
            $mensaje = $this->file_upload_error_message(
              $archivos["userfile"]["error"][$key]
            );
