...
class Usuario{
	...
	public function hacerLogin($parametros = array()){ //recibe $usuario, $contrasena para Login
...
class Ticket{
	...
	public function registrarTicket($parametros = array()){ //recibe ['datos_formulario']=>$_POST y ['archivo_adjunto']=>$_POST
		...
		$empresa = $oDatosEmpresa->obtenerEmpresa($parametros['datos_formulario']['id_empresa']);
	...
	public function obtenerSeguimientoTicket($param = array()){
		$consulta = "SELECT transiciones.*, usuarios.nombre
					FROM transiciones
					JOIN usuarios ON transiciones.intIdUsuario = usuarios.intIdUsuario
